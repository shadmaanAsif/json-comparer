...
| FR-005 |       P0 | Offer explicit array modes: ordered/indexed and unordered/multiset; display the active mode in results and exports.                                                                            |
| FR-006 |       P0 | Preserve original paths for both sides and use unambiguous internal path segments/JSON Pointer.                                                                                                |
| FR-007 |       P0 | Filter results by category and path, and apply validated ignore patterns: exact paths include descendants, `*` consumes one segment, and terminal `**` explicitly denotes a recursive subtree. |
...
| FR-013 |       P0 | Import a response through a controlled server endpoint using URL or supported cURL subset, showing method, target host, header names, redirects, status, media type, and truncation/errors.    |
| FR-014 |       P0 | Cancel local comparison and remote import operations.                                                                                                                                          |
| FR-015 |       P0 | Enforce configurable input, response, result-count, nesting, and execution-time limits with actionable errors.                                                                                 |
...
Controls: HTTPS by default; DNS resolution and every redirect revalidated; block loopback, link-local, private, multicast, metadata-service, non-HTTP(S), credential-in-URL, and disallowed-port targets; strip hop-by-hop headers; maintain header allowlist; never forward browser cookies; 10-second default timeout; decompressed byte cap; redirect cap; global/per-IP rate limits; abort on client disconnect where supported. Because DNS rebinding and proxy behavior are subtle, this endpoint requires focused security review before release.
...
6. Remote fetch rejects private/loopback/metadata addresses and unsafe redirects, enforces timeout/size/method/header policy, redacts secrets, and returns stable errors.
...
1. Are arrays ordered by default, unordered by default (artifact behavior), or selected per path?
2. Must URL/cURL import ship in MVP? Which methods, headers, target domains, and request-body sizes are allowed?
3. Is the deployment public internet, private/internal, on-premises, or desktop/offline?
4. Are accounts, saved comparisons, share links, collaboration, audit retention, or SSO MVP requirements?
5. What payload sensitivity/compliance regimes apply (PII, PHI, PCI, data residency), and may any telemetry leave the device?
6. What exact input sizes, nesting depths, performance reference devices, browser versions, and availability SLO apply?
7. Should structure comparison infer schemas from all array elements on both sides, and how should heterogeneous arrays be judged?
8. Are ignore rules glob-like, JSON Pointer-based, JSONPath-based, or all three via explicit modes?
9. Which export formats and report contents are mandatory, and may full changed values appear in exports?
10. What retention/deletion and backup/restore targets apply if cloud persistence is approved?
